...
from public.model.model_2D import *
...
def construct_network(input, model, initial_channel=64, rate=0.5):
    ...
    with tf.variable_scope('network'):
        predict = model(input, 1024, initial_channel=initial_channel, rate=rate, RNN_layer=2)
...
class train(object):
    ...
    def __init__(self, last, pattern, model_key, pb_path, ckpt_path, target, initial_channel, input_shape):
        self.graph = tf.Graph()
        self.last_flag = last
        ...
        self.valid_log_metric_only_path = 'build/{}-{}/valid_metric_loss_only.log'.format(target, model_key)
        self.detail_log_path = 'build/{}-{}/valid_detail.log'.format(target, model_key)
    ...
    def _train_graph_compose(self):
        if(self.pattern != "ckpt" and self.pattern != "pb"):
            ...
            with self.graph.as_default() as g:
                ...
                y = tf.placeholder(tf.float32, [None,self.input_shape[0], 1], name='label')
                keep_prob = tf.placeholder(tf.float32, name='rate')
                x = tf.placeholder(tf.float32, [None, *self.input_shape, 3], name='data')
                construct_network(x, self.model_dict[self.model_key], self.initial_channel, keep_prob)
