...
class train_valid_generator():
    def __init__(self, train_list, input_shape, batch_size, target='valence', ifrandom=True):
    ...
    def get_one(self):
        keys = list(self.record_dict.keys())
        index0 = random.randint(0, len(keys))
        index1 = random.randint(0, len(self.record_dict[keys[index0]])-self.time_step)
        data = [];label = []
        record_block = self.record_dict[keys[index0]][index1:index1+self.time_step]
        for item in record_block:
            ...
            if(self.target == 'valence'):
                ...
                assert False,'target selection flag must be v or a.'
            data.append(pic);label.append(va)
        ...
        for item in self.train_list:
            ...
            for one in item:
                ...
                if(self.target == 'valence'):
                    ...
                    assert False,'target selection flag must be v or a.'
                data_temp.append(pic)
                label_temp.append(va)

            data_batch_block.append(data_temp)
            label_batch_block.append(label_temp)    
            if(count == self.batch_size):
                yield data_batch_block,label_batch_block
                data_batch_block.clear()
                label_batch_block.clear()
